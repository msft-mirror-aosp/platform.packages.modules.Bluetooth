...
void smp_br_process_pairing_command(tSMP_CB* p_cb, tSMP_INT_DATA* p_data) {
  ...
  if (!p_dev_rec->is_bonded(BT_TRANSPORT_BR_EDR) ||
    ...
    log::warn("Not bonded over BR/EDR transport with SC {}", p_dev_rec->bd_addr);
    tSMP_INT_DATA smp_int_data{};
  ...
  if (!p_dev_rec->sec_rec.is_device_encrypted()) {
    log::warn("CKTD not allowed over unencrypted link {}", p_dev_rec->bd_addr);
    tSMP_INT_DATA smp_int_data{};
    smp_int_data.status = SMP_XTRANS_DERIVE_NOT_ALLOW;
    smp_br_state_machine_event(p_cb, SMP_BR_AUTH_CMPL_EVT, &smp_int_data);
    return;
  ...
  if (p_dev_rec->is_bonded(BT_TRANSPORT_LE) &&
    ...
    log::warn("Already bonded over LE with higher security {}", p_dev_rec->bd_addr);
    tSMP_INT_DATA smp_int_data{};
    smp_int_data.status = SMP_XTRANS_DERIVE_NOT_ALLOW;
    smp_br_state_machine_event(p_cb, SMP_BR_AUTH_CMPL_EVT, &smp_int_data);
    return;
